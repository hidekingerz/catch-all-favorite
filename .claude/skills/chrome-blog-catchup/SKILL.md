...
**このフィードは長期間更新が止まることがある**（実績: 2026-06-23 を最後に `lastBuildDate` が更新されず、1か月以上ブログ本体の新記事がフィードに反映されない状態が継続）。フィードだけを信じると新着を取りこぼすため、**必ずフィードの鮮度チェック（実行手順ステップ1）を行い、古い場合はステップ1bの一次探索を実施する。ステップ1bは「フォールバック」ではなく、フィード停止時の主経路である。**
...
続けて**フィードの鮮度を必ず確認する**: フィードの `lastBuildDate`（または最新記事の `pubDate`）が実行日から **7日以上前** の場合、フィードが更新停止している可能性がある。その場合は「新着なし」と結論せず、**ステップ1bの一次探索を必ず実施する**。

> **注**: 2026-06-23 以降、フィードの `lastBuildDate` が更新されない状態が続いている（実績で1か月以上停止）。このためフィードが 7 日以上古い場合は「フィードは参考情報」として扱い、**ステップ1bの探索結果を一次取得手段とする**。
