...
**このフィードは長期間更新が止まることがある**（実績: 2026-06-23 を最後に `lastBuildDate` が更新されず、2か月以上ブログ本体の新記事がフィードに反映されない状態が継続）。フィードだけを信じると新着を取りこぼすため、**必ずフィードの鮮度チェック（実行手順ステップ1）を行い、古い場合はステップ1bの一次探索を実施する。ステップ1bは「フォールバック」ではなく、フィード停止時の主経路である。**

> **代替フィード URL について（2026-08-14 確認済み）**: `https://developer.chrome.com/blog/feed.xml` および `https://developer.chrome.com/feed.xml` も同じ `lastBuildDate: 2026-06-23` を返すことを確認済み。代替 URL を試しても改善しない。フィードが 7 日以上古い場合は代替 URL の探索は不要で、すぐにステップ1bへ進むこと。
...
続けて**フィードの鮮度を必ず確認する**: フィードの `lastBuildDate`（または最新記事の `pubDate`）が実行日から **7日以上前** の場合、フィードが更新停止している可能性がある。その場合は「新着なし」と結論せず、**ステップ1bの一次探索を必ず実施する**。代替フィード URL（`/blog/feed.xml`・`/feed.xml`）も同じ停止日を返すため、代替 URL の確認は不要。

> **注**: 2026-06-23 以降、フィードの `lastBuildDate` が更新されない状態が続いている（実績で2か月以上停止、2026-08-14 時点）。このためフィードが 7 日以上古い場合は「フィードは参考情報」として扱い、**ステップ1bの探索結果を一次取得手段とする**。フィードが 7 日以上古い場合はステップ1bを省略してはならない。

### 1b. 一次探索: WebSearch + バージョンURLプローブ（フィード停止時の主経路）

フィードが 7 日以上古い場合に実施する。これはフォールバックではなく、フィード停止期間中の**主たる記事取得手段**である。**記事ページ自体はサーバーサイドレンダリングされている**ため、一覧さえ見つかれば curl / WebFetch で本文・公開日を取得できる。**(b) → (a) の順で実施すること**（WebSearch が全種別の記事を広くカバーするため先行する）。
...
一次探索でも新着が見つからなければ「新しい記事はありませんでした（フィード停止継続中、新記事未公開）」と報告する。**フィード停止を検知した場合は、`frontend-catchup-and-push` の不具合報告（issue起票）の対象として報告に含めること。**
