...
2. **URLは元ページに記載されているものをそのまま転記する**。URLを推測して書いてはいけない。ニュースレター内の各トピックには外部サイトへのリンク（GitHub、npm、ブログ記事等）が含まれている。それらの実際のURLを転記すること。`thisweekinreact.com/newsletter/XXX` 自体をURLとして使わない
3. **出力対象カテゴリ（React / Other）の全項目を漏れなく拾う**。各カテゴリにある項目は1つも省略せず、すべてMarkdownに含める（React Nativeは除外）
4. **存在しない記事を作らない**。一覧に載っていない号をでっち上げてはいけない。取得できた記事だけを使う
...
スケジュールタスクのプロンプト例:
```
This Week in Reactの最新号をキャッチアップして、Markdownファイルとして保存して
```
