...
1. **取得したページの内容だけを使う。** 自分の記憶や推測で速報・要約・日付・パッチレベルを補完しない
2. **URLはページに記載のものを使う。** URLを推測しない
3. **新着速報を漏れなく拾う。** 既存ファイルと重複しない速報はすべて対象にする
4. **新着がなければファイルを作らない。** すべて既存の場合は「新しい速報はありませんでした」と報告して終了する
...
| 記憶で速報内容を補完してしまう | 取得できた内容のみ使う。取得失敗時はスキップ |
| 既存速報まで重複してダイジェストに入れる | ステップ2の重複チェック（対象月）を必ず行う |
| 新着ゼロなのに空ファイルを作る | ファイルを作らず「新しい速報はありませんでした」と報告 |
