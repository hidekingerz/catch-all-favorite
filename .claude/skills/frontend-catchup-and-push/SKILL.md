...
12. `index.md` に新規ファイルへのリンクを追加
13. 作成されたファイルと更新した `index.md` を GitHub リポジトリへ **commit & push** し、`main` への **Pull Request を作成**
14. 作成した PR の **CI（GitHub Actions / ステータスチェック）を監視し、すべて成功したら main 保護をバイパスしてマージ**（レポート系PRに限る）
15. キャッチアップ中に検知した**スキル/取得処理の不具合**（RSSのURL変更・取得フォーマット崩れ・恒常的な取得失敗など）を GitHub issue として **自動起票**（重複は作らない）
...
ステップ1〜11で作成したファイル名が既にリポジトリに存在する場合は、そのファイルのpushをスキップする。新規ファイルのみをステップ13に進める。すべてのファイルが既に存在する（=新着なし）場合は「新しい記事はありませんでした」と報告して終了する。
...
### ステップ16: CI 監視とバイパスマージ

ステップ15で PR を作成（または既存PRを確認）したら、その PR の **CI（GitHub Actions / ステータスチェック）を監視し、すべて成功したら `main` へマージする**。`main` はブランチ保護（ruleset）されているため、**保護をバイパスしてマージ**する（リポジトリ管理者権限のトークンが前提）。

> **方針変更の明示**: このリポジトリの従来方針は「マージは管理者が手動で行う」だったが、本ステップではレポート系PRに限り CI 成功を条件に自動マージする。コード/CI/設定を含むPRは対象外（人手で確認する）。

**自動マージの対象条件（すべて満たす場合のみマージ）:**

- 今回作成・更新した PR の変更が **`content/` 配下と `index.md` のみ**（レポート系PR）。それ以外のファイルを含む場合はマージせず、報告して終了する
- CI のすべてのチェックが完了し、いずれも失敗していない

**監視手順:**

1. PR の head ブランチ最新コミットに紐づくチェックを取得する
   - GitHub MCP ツールが利用可能な場合: `mcp__github__pull_request_read`（対象PRのステータス/チェックを取得する method）で combined status とチェックランを確認する。補助的に `mcp__github__actions_list`・`mcp__github__get_check_run` を用いてもよい
   - 利用不可な場合は Bash で `gh pr checks <PR番号>` を試みる
2. すべてのチェックが**完了**するまで待つ。未完了（queued / in_progress）が残る間は短いバックオフでポーリングする（目安: 15s → 30s → 60s、最大待機 約10分）。`sleep` の乱用は避け、上記間隔に従う
3. 完了後に判定する:
   - すべて `success`（または `neutral` / `skipped` のみ）→ **マージへ進む**
   - 1つでも `failure` / `cancelled` / `timed_out` がある → **マージしない**。失敗内容を控え、必要ならステップ17で issue 化を検討し、ステップ18で報告して終了する
   - 待機上限を超えても完了しない → マージせず「CI 未完了のため手動確認が必要」と報告して終了する

**マージ:**

- GitHub MCP ツール（`mcp__github__merge_pull_request`）を使う
  - owner: `hidekingerz` / repo: `catch-all-favorite` / 対象PR番号
  - merge_method: `squash`（このリポジトリは squash 運用。失敗する場合はリポジトリ既定のマージ方式にフォールバックする）
  - コミットタイトル: PR タイトル（`chore: add frontend catchup YYYY-MM-DD`）
- 利用不可な場合は Bash で `gh pr merge <PR番号> --squash --admin`（保護バイパス）を試みる
- **ブランチ保護により API がマージを拒否した場合**（bypass 権限が無い等）は、無理にマージを繰り返さず「main 保護によりバイパスマージ不可。手動マージが必要」と報告して終了する。これは認証/権限の問題でありスキルの不具合ではないため issue 化はしない
- マージ成功後、可能であれば head の作業ブランチを削除する（`claude/...` のようなセッション指定ブランチは削除しない）
...
- pushしたファイル名
- 作成したPRのURL（該当する場合）
- **CI の結果とマージ可否**（マージ成功 / CI失敗のためマージせず / 保護によりバイパス不可で手動マージ待ち / CI未完了 のいずれか。該当する場合）
...
スケジュールタスクのプロンプト:
```
frontend-catchup-and-push スキルを実行して、jser.info・This Week in React・Chrome for Developers ブログ・Google 検索セントラル ブログ・Apple Developer News・iOS & iPadOS リリースノート・Android Security Bulletin・Android リリースノート・Apple セキュリティリリース・Google Play 最新情報・Claude Code をキャッチアップし、GitHubリポジトリにpushして PR を作成し、CI が成功したらバイパスマージまでして。
```
