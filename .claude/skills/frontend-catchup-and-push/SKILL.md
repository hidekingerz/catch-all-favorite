...
13. 作成されたファイルと更新した `index.md` を GitHub リポジトリへ **commit & push**
14. キャッチアップ中に検知した**スキル/取得処理の不具合**（RSSのURL変更・取得フォーマット崩れ・恒常的な取得失敗など）を GitHub issue として **自動起票**（重複は作らない）
...
### ステップ16: 改善項目の GitHub issue 化（不具合検知時・自動起票）
...
**issue 作成:** GitHub MCP ツール（`mcp__github__issue_write`）を使う。利用不可な場合はこのステップをスキップし、結果報告（ステップ17）で「未起票の不具合」として報告する。
...
スケジュールタスクのプロンプト:
```
frontend-catchup-and-push スキルを実行して、jser.info・This Week in React・Chrome for Developers ブログ・Google 検索セントラル ブログ・Apple Developer News・iOS & iPadOS リリースノート・Android Security Bulletin・Android リリースノート・Apple セキュリティリリース・Google Play 最新情報・Claude Code をキャッチアップし、GitHubリポジトリにpushして。
```
