...
> **重要**: このリポジトリの `main` はブランチ保護されており、直接 push は拒否される。そのため **必ず作業ブランチへ push し、ステップ14で PR を作成する**。`main` へ直接 push しようとしないこと。
...
### ステップ14: Pull Request 作成（必須・省略不可）

ステップ13で push したら、**必ず** その作業ブランチから `main` への Pull Request を作成する。新着が1件でもあって push が行われた場合、このステップを省略してはならない。push 後にターンを終えず、PR 作成まで必ず完了させること。
...
唯一の例外: ステップ11で**全ソースが新着なし**となり push 自体を行わなかった場合のみ、PR は作成しない（「新しい記事はありませんでした」と報告して終了）。
