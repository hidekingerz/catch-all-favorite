...
このソースのWeb取得は **3段階フォールバック**（ブラウザツール → `WebFetch` → Bash + `curl`）。対象ページは約 1.28MB の大容量 HTML のため、`WebFetch` では Markdown 変換中にテーブル到達前で切り詰められる（「Content truncated due to length」）ことがある。テーブル行が 1 件も取れなかった場合は Bash + `curl` で生 HTML を取得してパースする（詳細は実行手順ステップ 1 参照）。
...
1. **URLは `ja-jp` の日本語版を使う。** 各リリースの詳細ページ（例: `https://support.apple.com/ja-jp/124xxx`）はリンクに記載のURLをそのまま使う
