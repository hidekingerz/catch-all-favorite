...
1. **取得したページの内容だけを使う。** 自分の記憶や推測でリリース・要約・日付を補完しない
2. **URLはページに記載のものを使う。** URLを推測しない
3. **新着項目を漏れなく拾う。** 既存ファイルと重複しない項目はすべて対象にする
4. **新着がなければファイルを作らない。** すべて既存の場合は「新しいリリースノートはありませんでした」と報告して終了する
...
| 記憶で内容を補完してしまう | 取得できた内容のみ使う。取得失敗時はスキップ |
| 既存項目まで重複してダイジェストに入れる | ステップ2の重複チェック（日付＋見出し）を必ず行う |
