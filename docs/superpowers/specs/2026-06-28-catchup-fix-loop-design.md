...
[GitHub issue 起票]  label: catchup-maintenance（変更なし）
        │
        ▼
[routine: catchup-fix loop]  ← 新規。FullNetworkAccess 環境で定期ポーリング
...
- 他のスキル・`.claude/settings*`・`.github/`・`templates/`・ルート設定（`_config.yml` 等）。
...
2. **FAIL（修正が不十分）** — データ取れず → セッション内で最大 3 回、別アプローチで再修正・再 VERIFY。
...
### 環境配置でゲートを成立させる
- fix-loop routine は **FullNetworkAccess 環境**（`env_01WfQPJxjEDR1TQrYbb761pt`）で回す
  → `host_not_allowed` の多くを解消し VERIFY を実行可能にする。
- それでも到達不能なソースは INCONCLUSIVE として正直にエスカレーション。
...
- 対策: DISCOVER で「`loop-wip` だが紐付く PR もブランチも無く、ラベル付与から 24h 経過」した
  issue はスタックとみなし再選択可とする（付与時刻は issue のラベルイベント履歴で判定）。
...
| 環境 | FullNetworkAccess（`env_01WfQPJxjEDR1TQrYbb761pt`）|
